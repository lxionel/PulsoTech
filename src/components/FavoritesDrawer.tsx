        ...
        <div className="w-screen max-w-md bg-white border-l border-neutral-200 p-6 flex flex-col justify-between shadow-2xl relative text-neutral-900">
            ...
            {favoriteProducts.length === 0 ? (
              ...
              favoriteProducts.map((product) => {
                ...
                return (
                  <div
                    key={product.id}
                    className="flex gap-3.5 p-3.5 rounded-2xl bg-white border border-neutral-200/90 shadow-2xs hover:border-neutral-300 transition-all"
                  >
                    <Link
                      href={`/producto/${product.slug}`}
                      onClick={() => setIsFavoritesOpen(false)}
                      className="relative w-16 h-16 rounded-xl overflow-hidden bg-neutral-50 border border-neutral-100 shrink-0"
                    >
                      <Image
          ...
          {favoriteProducts.length > 0 && (
            <div className="pt-4 border-t border-neutral-200">
              <button
                onClick={() => setIsFavoritesOpen(false)}
                className="w-full py-3 rounded-xl bg-neutral-950 hover:bg-neutral-800 text-white font-bold text-xs flex items-center justify-center gap-2 shadow-xs transition-colors cursor-pointer"
              >
